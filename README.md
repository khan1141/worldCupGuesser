...
**Backend (optional)** — Spring Boot 4 + PostgreSQL. Seeds the 48 qualified teams on first startup and exposes the team API.

The frontend works fully standalone: it ships with a bundled team list and locally bundled flag images, so you can play — and deploy — without running the backend at all.
...
The backend adds server-side persistent team data. It is not required to run or deploy the app.
